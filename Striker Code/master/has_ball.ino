...
void orbitEnemy() {
  float angleError = motor.getRelativeAngle(0);
  if (abs(angleError) > 20) {
    motor.driveToRelativeHeadingCorrected(-80.0*orbitingConstant, 20.0*orbitingConstant, min(abs(angleError)*PIVOT_K, 80));
  } else {
    motor.stopMotors();
    isOrbiting = false;
  }
